...
var PuestoService = (function () {
    function PuestoService(http) {
        this.http = http;
        this.config = new config_service_1.Config();
    }
    PuestoService.prototype.send_data = function (datos, tipo) {
        ...
        this.path_url = this.config.puesto(tipo);
        // Petición HTML.
        return this.http.post(this.path_url, body, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    PuestoService.prototype.extractData = function (res) {
        var body = res.json();
        return body || {};
    };
    PuestoService.prototype.handleError = function (error) {
