...
var TareaService = (function () {
    function TareaService(http) {
        this.http = http;
        this.config = new config_service_1.Config();
    }
    TareaService.prototype.send_data = function (datos, tipo) {
        // ALGG 12-01-2016 Configuración de cabecera, cuerpo y opciones.
        var headers = new http_1.Headers({ "Content-Type": 'application/json' });
        var body = JSON.stringify({ datos: datos });
        var options = new http_1.RequestOptions({ headers: headers });
        // Dependiendo del tipo, se selecciona una URL u otra.
        this.path_url = this.config.tarea(tipo);
        // Petición HTML.
        return this.http.post(this.path_url, body, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    TareaService.prototype.extractData = function (res) {
        var body = res.json();
        return body || {};
    };
    TareaService.prototype.handleError = function (error) {
