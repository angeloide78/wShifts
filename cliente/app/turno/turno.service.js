...
var TurnoService = (function () {
    ...
    TurnoService.prototype.send_data = function (datos, tipo) {
        // ALGG 01-11-2016 Configuración de cabecera, cuerpo y opciones.
        var headers = new http_1.Headers({ "Content-Type": 'application/json' });
        var body = JSON.stringify({ datos: datos });
        var options = new http_1.RequestOptions({ headers: headers });
        // Dependiendo del tipo, se selecciona una URL u otra.
        this.path_url = this.config.turno(tipo);
        // Petición HTML.
        return this.http.post(this.path_url, body, options)
            .map(this.extractData)
            .catch(this.handleError);
    };
    TurnoService.prototype.extractData = function (res) {
        var body = res.json();
        return body || {};
    };
    TurnoService.prototype.handleError = function (error) {
